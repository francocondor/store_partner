            ...
            <div id="section-pedido">
              <div class="row">
                <div class="title-section-pedido text-center">
                  ...
                  <h3 style="margin-bottom: 15px;">Te estaremos llamando del 930123123</h3>
                    ...
                    <div class="col-xs-12 col-sm-7">
                      <div class="solicitud">
                        ...
                        <div class="info">
                          ...
                          <?php 
                          if($product->product_price==0 || $product->product_price==5)
                            $producto="chip" ;
                          ?>
                          @if(isset($product->variation_type_id))
                            @if($product->variation_type_id == 2)
                            <?php 
                            $precio1=$product->plan_price;
                            ?>
                            <p>Plan {{$product->plan_name}}* :<span> s/. {{$product->plan_price}} </span><!--mensual--></p>
                            @else
                            <?php $precio1=0;$prepago=1; ?>
                            <p>Plan Prepago <span>{{$product->plan_name}}</span></p>
                            @endif
                          @endif
                          @if(isset($product->promo_id))
                          <?php $precio2=$product->promo_price; ?>
                          <p>
                            Precio del {{$producto}}: <span>s/. {{$product->promo_price}}</span> <span class="normal-price">{{$product->product_price}}</span>
                          </p>
                          @else
                          <?php $precio2=$product->product_price; ?>
                          <p>
                            Precio del {{$producto}}: <span>s/. {{$product->product_price}}</span>
                          </p>
                          @endif
                          <p>................................................................................</p>
                          <p style="
...
">s/. {{ number_format($precio1 + $precio2, 2, '.', '') }}</span>
                          <?php if($prepago==0){ ?>
                          <p style="
                              margin-top: 10px;
                              font-size: 12px;
                              margin-bottom: 25px;
                          ">(*) Incluye renta adelantada</p>
                          <?php }else{ ?>                            
                          <p style="
                              margin-top: 10px;
                              font-size: 12px;
                              margin-bottom: 25px;visibility: hidden
                          ">(*) Incluye renta adelantada</p><?php 
                          } ?>
                        </div>
                      </div>
                    </div>
                ...
                <div class="rastreo-de-compra">
                  <div class="text-center">
                    ...
                    {{--<p>Un representante se comunicará contigo para confirmar tu pedido.</p>--}}
                    <p>Un representante se comunicará contigo para confirmar tu pedido.</p>
                  </div>
                </div>
