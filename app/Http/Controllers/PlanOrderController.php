...
class PlanOrderController extends Controller
{
  ...
  */
  ...
  // http://10.121.14.195:8236/BCCSWS/BCCSWSCommon?wsdl
  // Nuevo Ip
  ...
  protected function createConsultantRequest(&$order_detail) {
    ...
    try {
      ...
      if ($response->return->errorCodeMNP == '0') {
        ...
        Log::info('Respuesta bitelSoap 0');
      ...
      /****270519****/
        ...
        Log::info('Respuesta bitelSoap ELSE');
      }
      /****end 270519****/
  ...
  */
  ...
  //** INIT -Store Order Planes Flujo Rapido 
  public function storeOrderView (Request $request){
    ...
    if(count($request->session()->all()) == 0){
      return redirect()->route('home');
    }
    ...
    if($request->valuePlanControl != 4){

      $info_producto_variationID = DB::table('tbl_product_variation')
        ->where('product_id','=', $request->valueProductId)
        ->where('plan_id','=', $request->valueSelectTypePlan)
        ->where('affiliation_id','=',$request->valuePlanControl)
        ->where('contract_id','=', $request->valueContract)
        ->first();
  
        // SELECT * FROM bitel_ecommerce.tbl_product_variation where product_id = 103 and plan_id = 8 and affiliation_id = 2;
      // $getInfo_producto_variationID = $info_producto_variationID['product_variation_id'];

      $productGetPostpago = $this->shared->productPostpagoByStock($request->valueStockModelId,$info_producto_variationID->product_variation_id);

      // dump($productGetPostpago);
      // dump($info_producto_variationID );
      
  
        // dump($info_producto_variationID->product_variation_id);
        Log::info('Existe', (array) $info_producto_variationID);
        Log::info($info_producto_variationID->product_variation_id);
        // Log::info($info_producto_variationID['product_variation_id']);
    }else{
    ...
    Log::info('Type of affiliation (1=portabilidad 2=lineanueva 3:renueva tu plan 4:prepago): ', (array) $request->valuePlanControl);
    
    if($request->valuePlanControl != 4){

      $info_producto = DB::table('tbl_plan')->where('plan_id',$request->valueSelectTypePlan)->first();
      
      $affiliation = DB::table('tbl_affiliation')->where('affiliation_id',$request->valuePlanControl)->first();
      ...
      $info_producto = DB::table('tbl_plan')->where('plan_id',$request->valueSelectTypePlan)->first();
      // $affiliation = DB::table('tbl_affiliation')->where('affiliation_id',$request->valuePlanControl)->first();
      $order_detail['affiliation_type'] = 'Chip Prepago';


      
      // $equipo_plan = $request->valueSelectTypePlan;
    ...
    if($request->valuePlanControl == 1){
      if($request->type_number_carry == '02'){
        if($request->valueSelectArmaTuPlan){
          $order_detail['service_type'] = 'Postpago - Arma tu plan';
        }else{
          $order_detail['service_type'] = 'Postpago - Flujo Rapido';
        }
      }else{
        $order_detail['service_type'] = 'Prepago';
      }
    }else if($request->valuePlanControl == 2){
      $order_detail['service_type'] = 'Postpago';
    }else if($request->valuePlanControl == 3){
      $order_detail['service_type'] = $request->valueSelectTypePlanRenovacion;
    }else if($request->valuePlanControl == 4){
      
      $order_detail['service_type'] = $request->valueSelectTypePlanPrepago;
    }
    
    $order_detail['idtype_id'] = $request->document_type;
    $order_detail['payment_method_id'] = '4';
    ...
    if(isset($request->valuePlanControl) && isset($request->operator)){
      $source_operators = $this->shared->operatorList();
     ...
     //-- Aca se agregaría el webservices Planes

    if(isset($request) && \Config::get('filter.use_bcss')) { 
      // Apply validations with Bitel webservice before insert
      $this->initSoapWrapper(); // Init the bitel soap webservice
      Log::info('Ingreso al filter.use');
      if(isset($request->valuePlanControl) && $request->valuePlanControl == 3) {
        Log::info('Ingreso al plan tipo 3');
        if($this->checkIsRenovationUnavailable($order_detail)) {
          if (isset($order_detail['renov_ws_fail'])) {
            Log::info('renov_ws_fail isset');
            ...
            Log::info('renov_ws_fail No isset');
      ...
      if(isset($request->valuePlanControl) && $request->valuePlanControl == 1) {
        // process request portability

        Log::info('Ingreso al plan tipo 1');

        $responseCreateConsultantRequest=$this->createConsultantRequest($order_detail);

        Log::info($order_detail);

        Log::info($responseCreateConsultantRequest);
        ...
        if(is_bool($responseCreateConsultantRequest) && $responseCreateConsultantRequest==true) {
          ...
          Log::info('Ingreso al plan IF 1 responseCreateConsultantRequest ');
        }
          ...
          Log::info('Ingreso al plan IF 2 responseCreateConsultantRequest ');
        }
        else{     
            Log::error(json_encode([
            ...
            Log::info('Ingreso al plan IF 3 responseCreateConsultantRequest ');
        }
    ...
    if($request->valuePlanControl != 4){
      // igv no incluido en el precio
      // $order_detail['total'] = $info_producto->plan_price;
      // $order_detail['total_igv'] = $info_producto->plan_price + ($info_producto->plan_price * 0.18);
      // igv incluido en el precio
      $order_detail['total'] = $info_producto->plan_price;
      // $order_detail['total'] = $info_producto_variationID->product_variation_price;
      
      // $order_detail['total'] = $info_producto->plan_price - ($info_producto->plan_price * 0.18);
      $order_detail['total_igv'] = $info_producto->plan_price;
      // $order_detail['total_igv'] = $info_producto_variationID->product_variation_price ;
    }else{
      $order_detail['total'] = 5.00;
      $order_detail['total_igv'] = 5.00;
    }
    ...
    try{
      ...
      if(!empty($productGetPostpago->promo_id)){
        $promo_id = $productGetPostpago->promo_id;
      }else{
        $promo_id = null;
      }
      ...
      $order_items = [
        'stock_model_id' => $request->valueStockModelId,
        'product_variation_id' => $info_producto_variationID->product_variation_id,
        'promo_id' => $promo_id,
        'quantity' => 1,
        'subtotal' => number_format($order_detail['total'], 2, '.', ''),
        'subtotal_igv' => number_format($order_detail['total_igv'], 2, '.', ''),
        'equipo_plan' => 0,
        'order_id' => $order_id
      ];
    ...
    // return view('order_detail', ['products' => [], 'order_id' => $order_id, 'order' => $order_detail]);
    // dd($res);
    // dd($request->all());

    // dd($order_id);

    return view('page_pedidoRealizado',['order_id' => $order_id,'order' => $order_detail]);
    // return view('page_pedidoRealizado.blade',['order_id' => $order_id,'order' => $order_detail]);

  }
  //** END -Store Order Planes Flujo Rapido 
  
  //** INIT -Store Order Equipos Flujo Rapido 
  public function storeOrderViewEquipos (Request $request){
    ...
    if($request->valuePlanControl != 4){
      ...
      $productGetPrepago = $this->shared->productPrepagoByStock($request->valueStockModelId,$info_producto_variationID->product_variation_id);

      // dump($productGetPrepago);
    ...
    if(!empty($productGetPrepago->promo_id)){
      $promo_id = $productGetPrepago->promo_id;
    }else{
      $promo_id = null;
    }
    ...
    $order_detail['valueImageEquipo'] = $request->valueImageEquipo;
    $order_detail['valueBrandEquipo'] = $request->valueBrandEquipo;
    $order_detail['valueModelEquipo'] = $request->valueModelEquipo;
    $order_detail['valuePriceEquipo'] = $request->valuePriceEquipo;
    $order_detail['valueContract'] = $request->valueContract;
    ...
    //-- Aca se agregaría el webservices Equipos
    ...
    if($request->valuePlanControl != 4){
      ...
      $order_detail['total'] = $info_producto->plan_price;
      // $order_detail['total'] = $info_producto->plan_price - ($info_producto->plan_price * 0.18);
      $order_detail['total_igv'] = $info_producto->plan_price  + $request->valuePriceEquipo;
    }else{
      $order_detail['total'] = 5.00;
      $order_detail['total_igv'] = 5.00 + $request->valuePriceEquipo;
    }
    ...
    $stock_id = DB::table('tbl_stock_model')->where('product_id','=',$request->product_id)->first();
    ...
    return view('page_pedidoRealizadoEquipos',['order_id' => $order_id,'order' => $order_detail]);
    // return view('page_pedidoRealizado.blade',['order_id' => $order_id,'order' => $order_detail]);

  }
  //** END -Store Order Equipos Flujo Rapido
