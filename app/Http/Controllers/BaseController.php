...
class BaseController extends Controller
{
  ...
  public function searchProductPostpaid($category_id=1, $affiliation_id=1, $plan_id=7, $contract_id=1, $product_brands='', $pag_total_by_page=20, $pag_actual=1, $sort_by="", $sort_direction="", $product_price_ini=0, $product_price_end=0, $product_string_search="", $product_tag="", $product_ignore_ids = "", $only_best_variation = 0) {

    $products = DB::select('call PA_productSearchPostpago7(
      :product_categories,
      :product_brands,
      :affiliation_id,
      :plan_id,
    ...
    $total = DB::select('call PA_productSearchPostpago7(
      :product_categories,
      :product_brands,
      :affiliation_id,
      :plan_id,
    ...
    //return ['products' => $products, 'total' => $total[0]->total_products];
    return ['products' => $products, 'total' => count($total)];
  }
  ...
  public function getPostpaidChipValue() {
    ...
    IF(tbl_plan.plan_slug LIKE "%plus-29%", 1, 2) AS tiene_29'))
        ->orderBy('tiene_29')
...
//    return $result;
      ...
      /*->where(function ($query) use ($product, $affiliation_id, $contract_id) {
        $query
          ->where(function ($subquery) use ($product) {
            $subquery
              ->where('tbl_product_variation.affiliation_id', $product->affiliation_id)productPrepaidBySlug
  ...
  public function getProductPlansChipsFirst($product) {
    $affiliation_id = \Config::get('filter.affiliation_id');
    $contract_id = \Config::get('filter.contract_id');
    ...
    $result = DB::table('tbl_product_variation')
      ...
      ->select(DB::raw('tbl_plan.*, SUBSTRING(`plan_price`, 1, (POSITION("." IN `plan_price`) - 1)) AS plan_price_integer,
    SUBSTRING(`plan_price`, (POSITION("." IN `plan_price`) + 1), 2) AS plan_price_decimal,
    REVERSE(SUBSTRING(REVERSE(`plan_name`), POSITION(" " IN REVERSE(`plan_name`)) + 1, LENGTH(`plan_name`))) AS plan_name_name, tbl_affiliation.affiliation_id, tbl_affiliation.affiliation_slug, tbl_contract.contract_id, tbl_contract.contract_name, tbl_contract.contract_slug,IF(tbl_plan.plan_slug LIKE "%giga%", 2, 1) AS tiene_giga'))
      ->orderBy('tbl_plan.plan_price')
      ->orderBy('tiene_giga')
      ->orderBy('tbl_plan.weight')
      ->orderBy('tbl_plan.plan_id')
  ...
  public function getProductPlansFirst($product) {
    $affiliation_id = \Config::get('filter.affiliation_id');
    $contract_id = \Config::get('filter.contract_id');
    ...
    $result = DB::table('tbl_product_variation')
      ...
      ->orderBy('tbl_plan.weight')
      ->orderBy('tbl_plan.plan_id')
      ->select(DB::raw('tbl_plan.*, SUBSTRING(`plan_price`, 1, (POSITION("." IN `plan_price`) - 1)) AS plan_price_integer, SUBSTRING(`plan_price`, (POSITION("." IN `plan_price`) + 1), 2) AS plan_price_decimal, REVERSE(SUBSTRING(REVERSE(`plan_name`), POSITION(" " IN REVERSE(`plan_name`)) + 1, LENGTH(`plan_name`))) AS plan_name_name, tbl_affiliation.affiliation_id, tbl_affiliation.affiliation_slug, tbl_contract.contract_id, tbl_contract.contract_name, tbl_contract.contract_slug, IF(tbl_plan.product_code LIKE "%giga%", 2, 1) as extra_field'))
      ->get();
  ...
  public function getInfocomercialProductPlansType2($number_contactos,$number_gigabytes) {

   /*HVB - Se cambia la consulta para obtener GB y contactos por info_comercial*/
   
  $plans_filtered_BD = DB::table('tbl_plan')
   ->join('tbl_plan_infocomercial','tbl_plan.plan_id', '=', 'tbl_plan_infocomercial.plan_id')
   ->whereIn('tbl_plan_infocomercial.plan_infocomercial_descripcion', array('plan_gb'))
   ->where('tbl_plan.active', '=', 1)
   ->where('tbl_plan.plan_type', '=', 'Postpago')
   ->orderBy('tbl_plan.plan_price')
   ->select('tbl_plan.plan_id','tbl_plan.plan_name','tbl_plan.plan_price','tbl_plan.product_code','tbl_plan.plan_slug','tbl_plan.plan_bono','tbl_plan_infocomercial.plan_infocomercial_descripcion','tbl_plan_infocomercial.plan_infocomercial_informacion_adicional')
   ->get();

   
   /*HVB - Se transforma los arreglos de planes para que tengan como atributos contactos y GBs*/
   $plans_filtered = $this->buildPlanesArray($plans_filtered_BD);
   
   $array_pos = 0;
   $suggested_plans = array();
      
   while($array_pos < count($plans_filtered)){
	   
		 if ($plans_filtered[$array_pos]->plan_gb >= $number_gigabytes ) {
		   
		   if($array_pos == count($plans_filtered) - 1){
			   
				   if($array_pos - 2 >= 0){
					   $plan = new stdClass();
					   $plan = $plans_filtered[$array_pos - 2];
					   $plan->selected = false;
					   $plan->infoComercial = null;
					   $suggested_plans[] = $plan;
				   }
				   
				   if($array_pos - 1 >= 0){
					   $plan = new stdClass();
					   $plan = $plans_filtered[$array_pos - 1];
					   $plan->selected = false;
					   $plan->infoComercial = null;
					   $suggested_plans[] = $plan;
				   }
				   ...
				   $suggested_plans[] = $plan;
					...
					if($array_pos + 1 < count($plans_filtered)){
						$plan = new stdClass();
						$plan = $plans_filtered[$array_pos + 1];
						$plan->selected = false;
						$plan->infoComercial = null;
					...
					if($array_pos + 2 < count($plans_filtered)){
						$plan = new stdClass();
						$plan = $plans_filtered[$array_pos + 2];
						$plan->selected = false;
						$plan->infoComercial = null;
	...
	if(count($suggested_plans) == 0){
		
		$array_pos = count($plans_filtered) - 1;
	 ...
	 $array_pos = 0;
	 while($array_pos < count($suggested_plans)){
		 
		 $plan_details = DB::table('tbl_plan')
		 ->join('tbl_plan_infocomercial','tbl_plan.plan_id', '=', 'tbl_plan_infocomercial.plan_id')
		 ->where('tbl_plan.plan_id', $suggested_plans[$array_pos]->plan_id)
		 ->where('tbl_plan_infocomercial.active', 1)
		 ->orderBy('tbl_plan_infocomercial.plan_id')
		 ->orderBy('tbl_plan_infocomercial.weight')
		 ->select('tbl_plan_infocomercial.plan_infocomercial_img_url', 'tbl_plan_infocomercial.plan_infocomercial_descripcion', 'tbl_plan_infocomercial.plan_infocomercial_informacion_adicional', 'tbl_plan_infocomercial.plan_infocomercial_flag_cantidad','tbl_plan_infocomercial.weight','tbl_plan_infocomercial.tipo')
		 ->get();
		 
		$details_pos = 0;
		 while($details_pos < count($plan_details)){
			 if (!empty($plan_details[$details_pos]->plan_infocomercial_img_url)) {
			   $plan_details[$details_pos]->plan_infocomercial_img_url = asset(Storage::url(($plan_details[$details_pos]->plan_infocomercial_img_url)));
			 }
			 $details_pos++;
		 }
		 
		 $suggested_plans[$array_pos]->infoComercial =  $plan_details;
	...
	private function buildPlanesArray($arrayInput){
	   $array_pos = 0;
	   $resultArray = array();
	   
	   while ($array_pos < count($arrayInput)){
		
		$plan = new stdClass();
		
		
		if (strcasecmp($arrayInput[$array_pos]->plan_infocomercial_descripcion,'plan_gb') == 0)
		$plan->plan_gb = floatval($arrayInput[$array_pos]->plan_infocomercial_informacion_adicional);

			
		$plan->plan_id = $arrayInput[$array_pos]->plan_id;
		$plan->plan_name = $arrayInput[$array_pos]->plan_name;
		$plan->plan_price = $arrayInput[$array_pos]->plan_price;
		$plan->product_code = $arrayInput[$array_pos]->product_code;
		$plan->plan_slug = $arrayInput[$array_pos]->plan_slug;
		$plan->plan_bono = $arrayInput[$array_pos]->plan_bono;
		
		Log::info('plan Build: ', (array) $plan);
		
		$resultArray[] = $plan;
		
		//Se avanza 1 porque solo busca plan_gbs y ya no plan_contactos (antes era + 2)
  ...
  public function getPlans($plan_type = null) {
    ...
    $query = DB::table('tbl_plan')
      ->select(DB::raw('*,CASE 
          WHEN plan_name LIKE "%sico%" THEN 1 
          WHEN plan_name LIKE "%hip%"  THEN 2 
          WHEN plan_name LIKE "%lash%"  THEN 3 
          ELSE 4 END AS order_factor'))
      ->where('active', 1)
      ->orderBy('plan_price')
      ->orderBy('order_factor')
      ->orderBy('weight')
      ->orderBy('plan_id');
