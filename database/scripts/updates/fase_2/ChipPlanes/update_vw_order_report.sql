...
VIEW `vw_order_report` AS
  ...
  CONCAT(`ord`.`first_name`,' ',`ord`.`last_name`) AS `Nombre Cliente`,
  ...
  `itm`.`equipo_plan`         AS `Equipo`,
  `pro`.`product_model`       AS `Modelo`,
  (SELECT
     `ors`.`order_status_name`
   FROM (`tbl_order_status_history` `osh`
      JOIN `tbl_order_status` `ors`
        ON ((`osh`.`order_status_id` = `ors`.`order_status_id`)))
...
FROM (((`tbl_order` `ord`
    ...
    JOIN `tbl_district` `dst`
      ON ((`ord`.`delivery_district` = `dst`.`district_id`)))
   JOIN (`tbl_order_item` `itm`
         LEFT JOIN ((`tbl_product_variation` `var`
                     JOIN `tbl_plan` `pln`
                       ON ((`var`.`plan_id` = `pln`.`plan_id`)))
                    JOIN `tbl_product` `pro`
                      ON ((`var`.`product_id` = `pro`.`product_id`)))
           ON ((`itm`.`product_variation_id` = `var`.`product_variation_id`)))
     ON ((`ord`.`order_id` = `itm`.`order_id`)))
WHERE ((`ord`.`service_type` = 'Accesorios')
