...
class CartController extends Controller
{
  ...
  public function addToCart (Request $request) {
    ...
    switch ($cart_item['type_id']) {
      ...
      case 2:
        ...
        if(!$this->validateVariationAffiliation($cart_item['product_variation_id'],$cart_item['affiliation_id']))
          return redirect()->back()->with('msg', 'Ocurrió un error. Por favor intente nuevamente.');
        $has_item = $cart->search(function($item, $key) use ($cart_item) {
          return $item['product_variation_id'] == $cart_item['product_variation_id'] && $item['stock_model_id'] == $cart_item['stock_model_id'];
        });
        if ($has_item === false && !$cart->contains('type_id', 1) && !$cart->contains('type_id', 2)) {
  ...
  public function removeFromCart (Request $request) {
    ...
    Log::info('cart array: ', (array) $cart);
    // prepago
  ...
  public function validateVariationAffiliation($product_variation,$affiliation) {

    $results = DB::select( DB::raw("SELECT * FROM tbl_product_variation WHERE product_variation_id = '$product_variation' and affiliation_id='$affiliation'") );
